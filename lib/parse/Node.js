...
for (i in NodeType)
{
	...
	switch (NodeType[i])
	{
		...
		case NodeType.HBS_EXPRESSION_PATH:
		{
			...
			{
				var node = { type:NodeType.HBS_EXPRESSION_PATH };
				
				if (statement.path.parts != null)
				{
					node.value = statement.path.parts;
				}
				else
				{
					// Place literals in array for consistency
					node.value = [statement.path.value];
				}
				
				if (statement.path.data === true) node.data = true;
