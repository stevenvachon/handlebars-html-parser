...
{
	this.preformattedElementStack.push(true);
}

function addState()
{
	this.stateStack.push( state.createNew() );
}
...
{
	return this.preformattedElementStack.length > 0;
}
...
function removePreformatted()
{
	this.preformattedElementStack.pop();
}
...
function parse(template, options)
{
	return new Promise( function(resolve, reject)
	{
		var env = 
		{
			...
			addPreformatted: addPreformatted,
			addState: addState,
			...
			removePreformatted: removePreformatted,
			removeState: removeState,
			
			// Stacks
			hbsProgramStack: [],
			preformattedElementStack: [],
			stateStack: [],
		...
		env.parseHtml().then( function()
		{
			//devlog(env.outputProgram);
			resolve(env.outputProgram);
		});
	});
}
