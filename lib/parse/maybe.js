...
function maybeConcatOrPushTextNode(part, env)
{
	...
	if (lastNode!==undefined && lastNode.type===NodeType.TEXT)
	{
		// Concatenate
	...
	else
	{
		...
		if (lastNode!==undefined && lastNode.stripWhitespace===true)
		{
			// Strip text on left
			part = trimLeft(part);
		}
...
function maybePopEmptyTextNode(env)
{
	...
	if (lastNode!==undefined && lastNode.type===NodeType.TEXT && lastNode.value==="")
	{
