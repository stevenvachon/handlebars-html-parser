...
function parseHtml(env)
{
	...
	return eachChild(html.childNodes, env).then(() =>
	{
		env.removeState();
	});
}
...
function pushTag(childNode, env)
{
	...
	var isPreformatted = env.options.preformattedElements[childNode.tagName] === true;
	if (isPreformatted === true) env.addPreformatted();
	...
	.then(() =>
	{
		...
		if (isPreformatted === true) env.removePreformatted();
	});
}
