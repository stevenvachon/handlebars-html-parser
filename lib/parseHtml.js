...
{
	var i,len,newNodeStart,part,partType;
	var inverted = null;
	var parts = splitAliases(text);
	var statementProgram = null;
		...
		if (partType === "number")
		{
			...
			switch (statement.type)
			{
				case "BlockStatement":
				{
					...
					{
						statementProgram = statement.program;
						inverted = false;
					}
					// `{{^path}} {{/path}}`
					else if (statement.inverse !== undefined)
					{
						statementProgram = statement.inverse;
						inverted = true;
					}
					else
					{
						statementProgram = null;
						inverted = null;
					}
					...
					if (statementProgram !== null)
					{
						...
						if (statementProgram.body.length > 0)
						{
							...
							thisObj.newProgram.push( newNodeStart );
							thisObj.newProgram.push( Node.HBS_EXPRESSION(statement, true) );
							thisObj.newProgram.push( Node.HBS_TAG_END(statement, true) );
						}
						else
						{
